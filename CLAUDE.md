...
- **The suite and the harness share one batter.** `QUALITY_QUANTILES` / `SPRAY_MEAN_DEG` / `BATTED_BALL_MIX` and `sweep()` live here; `tests/conftest.py` re-exports the samplers under their old names. A band asserted in a test and a number quoted in a doc can only mean the same thing if one definition produced both — CLAUDE.md records four miscalibrations caused by drifting copies of exactly these numbers.
...
- **There is no HR column, and that is not an oversight.** The harness forces `IN_PLAY`, and the home-run roll happens upstream in `HitOutcomeManager._resolve_outcome`, so a sweep here can never produce one.
...
- **Next calibration item, measured but not tuned:** the ground-ball hit rate is **29.8%** at LEAGUE against MLB's ~24%, and BABIP **.324** against ~.300. `body_block` is the named dial, but it and `field_misplay_p` interact — raising the block removes through-balls, which are also most of the errors — so it is a two-variable fit, not another single pass. The infield-hit rate is in band at **7.9%** (MLB 6-8%).
...
- **Recorded play from before a contact-geometry change describes a different
  ...
  - **Four places are anchored to those same quantiles and must move
    ...
    `QUALITY_QUANTILES` / `SPRAY_MEAN_DEG` (the shared samplers, re-exported
